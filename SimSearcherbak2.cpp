...
//						cout << mid << endl;
						mid = (h + t) / 2;
						if ((*jaccard_list[candid_list[j].second])[mid] <= i) h = mid; else t = mid;
					}
					if ((*jaccard_list[candid_list[j].second])[h] == i) cnt ++;
					if (cnt >= thres)
						break;
				}
				if (cnt >= thres)
			...
			for (int i = 0; i < context.size(); i++)
			{
				if (candid_set[i] > 0)
				{
					...
					for (int j = tmp; j < candid_list.size(); j++)
					{
						...
						{
							mid = (h + t) / 2;
							if ((*ed_list[candid_list[j].second])[mid] <= i) h = mid; else t = mid;
						}
						if ((*ed_list[candid_list[j].second])[h] == i) cnt ++;
						if (cnt >= thres)
							break;
					}	
					if (cnt >= thres)
